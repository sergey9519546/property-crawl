...
5. **Air-Gapped Two-Tier Notice Intelligence**: Legal notice parsing enforces OCR error recovery (`$12O,OOO` -> `120000`), multi-parcel separation, prompt injection defense, and air-gapped deterministic regex extraction with 50-state statutory redemption rules and senior lienholder risk detection (`server/ai/notice-parser.js`, `server/ai/legal-rules.js`).
6. **Scraper Circuit Breaker Protection**: All data collectors enforce `ScraperCircuitBreaker` (`server/scrapers/circuit-breaker.js`) to trip on 403 Forbidden, Cloudflare challenge walls, or corrupted/truncated feeds, ensuring live listings are never overwritten with poison payloads.
