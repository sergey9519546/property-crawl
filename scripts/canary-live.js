 ...
 *   - accepted records, zero source errors, complete + fullSweep, not truncated
 *
 * Requires PostgreSQL + DISCOVERY_MODE=advanced for live runs.
 * Without DATABASE_URL the CLI prints a dry-run plan and exits 0.
 *
 * Usage:
 *   node scripts/canary-live.js list
 *   node scripts/canary-live.js status
 *   node scripts/canary-live.js status --source servicelink
 *   node scripts/canary-live.js run --all-promoted
 *   node scripts/canary-live.js run --sources treasury,usda
 *   node scripts/canary-live.js run --wave wave1 --repeat 2
...
/** Clean canary = Migration 014 gate-clean criteria used by discovery-worker. */
...
async function loadDiscoveryStack(env = process.env) {
  ...
  return { db, store, worker, scheduler };
}
...
async function cmdList() {
  ...
  const { store } = await loadDiscoveryStack();
...
async function cmdRun(flags) {
  ...
  const { db, store, worker, scheduler } = await loadDiscoveryStack();
  let promoted = [];
  try {
  ...
  // Optional Scrapling structural parser for this canary batch.
  if (flags.scrapling === true || flags['scrapling-sources']) {
    const list = flags['scrapling-sources']
      ? parseSourceList(flags['scrapling-sources']).join(',')
      : targets.sources.join(',');
    process.env.SCRAPLING_SOURCES = list;
    console.log(`[canary] SCRAPLING_SOURCES=${list}`);
  }
  ...
  for (const source of targets.sources) {
    for (let attempt = 1; attempt <= repeat; attempt += 1) {
      ...
      console.log(`[canary] ${source} attempt ${attempt}/${repeat}: ${mark}${evaluation.accepted != null ? ` accepted=${evaluation.accepted}` : ''}${error ? ` error=${error}` : ''}`);
  ...
  console.log('[canary] clean counts (need ≥2 distinct runs for promote):');
  for (const source of targets.sources) {
    console.log(`  ${source}: ${cleanBySource.get(source) || 0}`);
  }

  db.pool?.end?.();
  return results.some((r) => r.evaluation?.clean) ? 0 : 1;
}
...
Clean criteria (gate-clean):
  sole source, accepted > 0, no source/observation error,
