...
function synthesizeCadastralParcel(lat, lng, sqft, apn, customProps = {}) {
  ...
  // Slight angle offset (+0.00001) gives realistic surveyor property orientation
  const angle = 0.08; // ~4.5 degrees off cardinal grid
...
async function queryArcGisRest(lat, lng, state, county, sqft, apn, listing) {
  ...
  if (serviceInfo && typeof fetch !== 'undefined') {
    ...
    try {
      ...
      // Gracefully catch timeout / abort / network error and proceed to fallback
    } finally {
      clearTimeout(timeout);
    }
  }
