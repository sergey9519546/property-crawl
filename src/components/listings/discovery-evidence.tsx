...
export function DiscoveryEvidence({ evidence }: { evidence: DiscoveryEvidenceData }) {
  ...
  return <div className="space-y-5 text-xs">
    {sale && <details className="border-b border-slate-200 pb-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">Sale timing in source records</summary><dl className="mt-3 grid gap-3 sm:grid-cols-2">
    ...
    {timeline && <details className="border-b border-slate-200 pb-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">Source history · {timeline.total} saved records</summary>
      ...
      <ol className="mt-3 space-y-3">{timeline.items.map(item => <li key={item.snapshotId} className="border-l-2 border-slate-200 pl-3"><p className="font-semibold">{date(item.observedAt)} · {item.origin === 'archive' ? 'Dated archive' : item.origin}</p><p className="mt-1 text-slate-500">{item.kind === 'closed_result' ? 'Closed-result summary; completed sale remains unverified' : 'Publisher record snapshot'}</p><a className="mt-1 inline-block underline" href={item.evidenceUrl} target="_blank" rel="noreferrer" onClick={(event) => { if (!session.authenticated) { event.preventDefault(); session.requestUnlock(); } }}>Inspect raw evidence · {item.payloadSha256.slice(0, 12)}</a></li>)}</ol>
