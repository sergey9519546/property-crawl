...
import { ArrowRight, Download, ExternalLink, Loader2, Radar } from 'lucide-react';
...
export function PropertyIntelligence({ listingId }: { listingId: string }) {
  ...
  const publicRequest = useRef<AbortController | null>(null);
  const researchRequest = useRef<AbortController | null>(null);
  const loadPublic = useCallback(async (requestGeneration: number) => {
    publicRequest.current?.abort();
    const controller = new AbortController(); publicRequest.current = controller;
    setBusy(true); setError('');
    try {
      const response = await fetch(`/api/property-intelligence?listingId=${encodeURIComponent(listingId)}`, { cache: 'no-store', signal: controller.signal });
      ...
      setDossier(result);
    } catch (caught) { if (!(caught instanceof DOMException && caught.name === 'AbortError') && requestGeneration === generation.current) setError(caught instanceof Error ? caught.message : 'Property investigation could not be completed'); }
    finally { if (requestGeneration === generation.current && publicRequest.current === controller) setBusy(false); }
  }, [listingId]);
  useEffect(() => { const requestGeneration=++generation.current;researchRequest.current?.abort();setDossier(null);void loadPublic(requestGeneration);return()=>{publicRequest.current?.abort();researchRequest.current?.abort();}; }, [loadPublic]);

  async function research() {
    if (!session.authenticated) { session.requestUnlock(); return; }
    researchRequest.current?.abort(); const controller=new AbortController();researchRequest.current=controller;
    const requestGeneration=generation.current;setBusy(true);setError('');
    try {
      const response=await fetch('/api/property-intelligence',{method:'POST',cache:'no-store',credentials:'same-origin',signal:controller.signal,headers:{'Content-Type':'application/json'},body:JSON.stringify({listingId})});
      if(response.status===401){await session.refresh();session.requestUnlock();throw new Error('Your workspace session expired. Unlock it to check public records.');}
      const result=await response.json();if(!response.ok)throw new Error(result.error||'Property investigation could not be completed');
      if(requestGeneration!==generation.current||controller.signal.aborted)return;setDossier(result);
    } catch(caught){if(!(caught instanceof DOMException&&caught.name==='AbortError')&&requestGeneration===generation.current)setError(caught instanceof Error?caught.message:'Property investigation could not be completed');}
    finally{if(requestGeneration===generation.current&&researchRequest.current===controller)setBusy(false);}
  }
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="Property intelligence dossier">
    <div className="border-b border-slate-200 p-5 sm:p-6">
      <h2 className="text-xl font-semibold tracking-tight text-slate-950">Research &amp; history</h2>
      <p className="mt-2 text-sm leading-6 text-slate-600">Review changes, compare public records, and check missing details.</p>
      <div className="mt-4 flex flex-wrap gap-3"><button onClick={() => void research()} disabled={busy || dossier?.source.status !== 'source_observed'} className="flex min-h-11 items-center gap-2 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{busy ? <Loader2 size={15} className="animate-spin" /> : <Radar size={15} />}{busy ? 'Checking records…' : 'Check public records'}</button><button onClick={exportDossier} disabled={!dossier} className="flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 disabled:opacity-40"><Download size={14} /> Download evidence</button></div>
    </div>
    <div className="space-y-5 p-5 sm:p-6">
      ...
      {dossier && <>
        ...
        <div className="rounded-xl bg-slate-50 p-4"><h3 className="text-sm font-semibold text-slate-950">Changes &amp; discrepancies</h3>
          ...
          </div> : <p className="mt-2 text-sm leading-6 text-slate-600">{dossier.historyUnavailable ? 'Observation history is temporarily unavailable.' : dossier.history ? `${dossier.history.observations} source ${dossier.history.observations === 1 ? 'observation' : 'observations'} since ${date(dossier.history.firstObservedAt)}. No recorded changes or discrepancies found.` : 'No comparison history yet. Future source captures will show changes here.'}</p>}
        </div>
        <details className="border-b border-slate-200 pb-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">All recorded facts · {dossier.facts.length}</summary><dl className="mt-3 divide-y divide-slate-100">{dossier.facts.map((fact) => <div key={fact.key} className="grid gap-1 py-2.5 text-sm sm:grid-cols-[1fr_1.4fr]"><dt className="text-slate-500">{fact.title}</dt><dd className="break-words font-medium">{valueLabel(fact.value, fact.key)}{fact.evidenceClass === 'unverified_snapshot' && <span className="ml-2 text-amber-700">Unverified snapshot</span>}</dd></div>)}</dl></details>
        {dossier.publicRecords && <details className="border-b border-slate-200 pb-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">Public records &amp; neighborhood</summary><div className="mt-4 space-y-4">
        ...
        <details className="border-t border-slate-200 pt-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">What still needs checking · {dossier.gaps.length}</summary><div className="mt-4 space-y-4">{dossier.gaps.map((gap) => <div key={gap.id}><h4 className="text-sm font-semibold">{gap.title}</h4><p className="mt-1 text-sm leading-6 text-slate-600">{gap.reason}</p><p className="mt-1 text-sm leading-6 text-emerald-800">Next: {gap.nextAction}</p></div>)}</div></details>
      </>}
    </div>
  </section>;
}
