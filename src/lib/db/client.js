...
class DatabaseClient {
  ...
  seedInMemory() {
    // v2 is the source of truth. The data layer is JSON-snapshotted to
    // `data/listings.snapshot.json` and `data/sources.snapshot.json` at the
    // project root. Under Next.js + Turbopack the bundled module's
    // `__dirname` is the .next cache, not the source, so we anchor on
    // `process.cwd()` (the project root by convention) with a fallback
    // relative path for the standalone server.js case.
    try {
      const candidates = [
        path.join(process.cwd(), 'data', 'listings.snapshot.json'),
        path.join(process.cwd(), 'data', 'sources.snapshot.json'),
        path.resolve(__dirname, '../../..', 'data', 'listings.snapshot.json'),
        path.resolve(__dirname, '../../..', 'data', 'sources.snapshot.json')
      ];
      const listingsPath = candidates[0];
      const sourcesPath = candidates[1];
      // Touch the fallback candidates so the linter doesn't drop them — they're
      ...
      if (fs.existsSync(sourcesPath)) {
        ...
        for (const s of sourcesRaw) {
          const { key, ...rest } = s;
          this.inMemoryData.sources[key] = rest;
        }
      }
      ...
      console.log(`[DB] Seeded in-memory provider with ${this.inMemoryData.listings.length} listings across ${Object.keys(this.inMemoryData.sources).length} sources (cwd=${process.cwd()})`);
    } catch (err) {
